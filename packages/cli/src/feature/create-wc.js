...
const TEMPLATE_URL = "../templates/wc:create/"
...
export function createWC(webComponent) {
  ...
  // css
  const templateThemeCss = readFileSync(path.resolve(__dirname, `${TEMPLATE_URL}src/css/theme.css.ts.ejs`), 'utf8');
  const ThemeCssContent = ejs.render(templateThemeCss, bindDataObj);
  writeFileSync(cssFilePath, ThemeCssContent);


  // viewmodel
  const templateViewModel = readFileSync(path.resolve(__dirname, `${TEMPLATE_URL}src/viewmodel.ts.ejs`), 'utf8');
  const ViewModelContent = ejs.render(templateViewModel, bindDataObj);
  writeFileSync(viewModelFilePath, ViewModelContent);

  // view 
  const templateView = readFileSync(path.resolve(__dirname, `${TEMPLATE_URL}src/view.ts.ejs`), 'utf8');
  const viewContent = ejs.render(templateView, bindDataObj);
  writeFileSync(viewFilePath, viewContent);

  // story
  const templateIndexStory = readFileSync(path.resolve(__dirname, `${TEMPLATE_URL}stories/index.stories.ts.ejs`), 'utf8');
  ...
  // package.json
  const templatePackageJson = readFileSync(path.resolve(__dirname, `${TEMPLATE_URL}package.json.ejs`), 'utf8');
  const packageJsonContent = ejs.render(templatePackageJson, { wcName });
  writeFileSync(packageJsonFilePath, packageJsonContent);

  // index.ts
  const templateIndex = readFileSync(path.resolve(__dirname, `${TEMPLATE_URL}index.ts.ejs`), 'utf8');
